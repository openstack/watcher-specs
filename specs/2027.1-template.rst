...
* If this change introduces new database schema changes, describe the
  migration strategy. Will existing audit records, action plans, or
  strategy configurations need to be migrated? Consider the impact on
  operators who may have long-lived audit data they need to preserve.

* If this change adds or modifies a strategy plugin, consider whether
  existing audit templates referencing the old strategy parameters will
  continue to work. Will operators need to update their audit templates
  or goal-to-strategy mappings after the upgrade?

* If this change modifies how Watcher interacts with external services
  (e.g. Nova, Cinder, Neutron, or Monasca/Gnocchi), consider whether
  the new behavior requires minimum versions of those services. Does
  the change gracefully handle environments where the external service
